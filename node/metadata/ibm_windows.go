...
var (
	ibmInstanceMetadataAddress = "api.metadata.cloud.ibm.com"
	ibmAZSuffix                = regexp.MustCompile(`-(\d+)$`)
)
...
func getIBMMetadata() *CloudMetadata {
	...
	return &CloudMetadata{
		...
		AccountId:        ibmAccountIDFromCRN(md.CRN),
		InstanceId:       md.Id,
		Region:           ibmAZSuffix.ReplaceAllString(md.Zone.Name, ""),
		AvailabilityZone: md.Zone.Name,
		InstanceType:     md.Profile.Name,
		LifeCycle:        firstNonEmpty(md.LifeCycle, md.LifeCycleState),
	}
}

func ibmAccountIDFromCRN(crn string) string {
	parts := strings.Split(crn, ":")
	for _, part := range parts {
		if accountID, ok := strings.CutPrefix(part, "a/"); ok {
			return accountID
		}
	}
	return ""
