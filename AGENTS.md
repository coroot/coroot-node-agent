...
> operating rules: build-tag discipline, plan-driven implementation,
> doc sync, and the pre-commit sanity gates.
...
## Workflow: plans → code → plan-complete
...
1. **Update the plan doc.** Edit (or create) the relevant
   ...
   plan isn't ready for implementation.

2. **Implement against the plan.** Keep work scoped to the relevant
   plan section and update the plan doc in the same commit when
   behavior shifts.

3. **Close the plan when criteria are met.** Once every item in the
   plan's Acceptance Criteria is demonstrably satisfied (tests pass,
   behavior verified), update the plan doc to mark it complete
   (status header or a `Status: Complete` line at the top). A plan is
   complete only when the acceptance criteria are satisfied.
...
work that goes through the plan-driven implementation loop, the
relevant plan doc must be updated **before** or **alongside** the code
change.
