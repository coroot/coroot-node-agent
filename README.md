...
Windows implementation work is complete for the M0-M5 milestones on
this branch. Windows builds support startup, node-level metrics,
Docker-based container discovery, native Windows service execution, an
ETW-backed TCP scaffold for process-isolated Windows Docker containers,
and Docker `json-file` stdout/stderr log-pattern metrics. Final
support still requires Windows Server 2022 validation; see
`docs/windows-support-matrix.md`. Windows service installation is
documented in `docs/windows-quickstart.md`.
